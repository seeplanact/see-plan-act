...
export default function AdminBlogList() {
  ...
  const fetchBlogs = useCallback(
    () => adminBlogsAPI.getAll({ page, limit: 15, search, status }),
    [page, search, status]
  );

  const { data, loading, refetch } = useFetch(fetchBlogs, [page, search, status]);
  ...
  const handleToggle = async (id, current) => {
    setTogglingId(id);
    try {
      await adminBlogsAPI.togglePublish(id);
      toast.success(current ? 'Moved to drafts' : 'Blog published');
      refetch();
    } catch (err) {
      toast.error(err.message);
    } finally {
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      >
        <div>
          <p className="text-white/30 font-mono text-xs tracking-widest uppercase mb-1">Content</p>
          <h1 className="font-display font-bold text-2xl sm:text-3xl text-white">All Blogs</h1>
        ...
        <Link to="/admin/blogs/new" className="btn-primary flex items-center gap-2 text-sm self-start">
          <HiOutlinePlusCircle size={16} /> New Post
        </Link>
      </motion.div>
      ...
      {loading ? (
        ...
        <div className="bg-brand-card border border-brand-border rounded-sm p-12 text-center">
          <p className="text-white/30 font-body text-sm mb-4">
            {search || status ? 'No posts match your filters.' : 'No blog posts yet.'}
          </p>
          {!search && !status && (
            <Link to="/admin/blogs/new" className="btn-primary text-sm">Create your first post</Link>
          )}
        </div>
      ) : (
        <>
          <div className="bg-brand-card border border-brand-border rounded-sm overflow-hidden">
            {/* Desktop header row */}
            <div className="hidden sm:grid grid-cols-[1fr_110px_90px_70px_90px] gap-4 px-6 py-3 border-b border-brand-border">
            ...
            {blogs.map((blog, i) => (
              <motion.div
                key={blog._id}
                className={`sm:grid sm:grid-cols-[1fr_110px_90px_70px_90px] gap-4 px-4 sm:px-6 py-4
                  hover:bg-white/2 transition-colors
                  ${i < blogs.length - 1 ? 'border-b border-brand-border' : ''}`}
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                transition={{ delay: i * 0.03 }}
              >
                <div className="min-w-0 mb-2 sm:mb-0">
                  <p className="text-white/90 font-body text-sm truncate">{blog.title}</p>
                  <p className="text-white/25 font-mono text-xs mt-0.5">{formatDate(blog.updatedAt)}</p>
                </div>

                <div className="flex items-center mb-1 sm:mb-0">
                  <span className="text-white/40 font-mono text-xs capitalize">
                ...
                <div className="flex items-center mb-1 sm:mb-0">
                  <button
                    onClick={() => handleToggle(blog._id, blog.isPublished)}
                  ...
                  >
                    {togglingId === blog._id ? '...' : blog.isPublished ? 'Live' : 'Draft'}
                  </button>
                </div>

                <div className="flex items-center mb-2 sm:mb-0">
                  <span className="text-white/30 font-mono text-xs">{blog.views ?? 0}</span>
                </div>
          ...
          {totalPages > 1 && (
            <div className="flex justify-center items-center gap-3 mt-6">
              <button
                ...
                disabled={page === 1}
                className="px-4 py-2 border border-brand-border text-white/40 text-sm font-mono
                           hover:border-brand-cyan hover:text-brand-cyan disabled:opacity-30
                           disabled:cursor-not-allowed transition-all rounded-sm"
              >
                ← Prev
              </button>
